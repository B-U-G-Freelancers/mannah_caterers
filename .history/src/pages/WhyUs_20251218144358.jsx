...
import { Leaf, Clock, ChefHat, Users, Utensils, ArrowRight } from "lucide-react";
import { motion } from "framer-motion";
import { Routes, Route, Link } from "react-router-dom";
import Whyheader from ""
export default function WhyUs() {
    ...
    <div className="min-h-screen bg-[#0c120c] text-white selection:bg-[#e2b857] selection:text-black overflow-hidden">
      ...
      <div className="relative">
        ...
        <div 
          ...
          style={{
            backgroundImage: 'url("https://partyslate.imgix.net/photos/1734724/photo-a9678605-fbaf-49bd-8ba0-ea710f19c02f.jpg?ixlib=js-3.8.0&auto=compress%2Cformat&bg=fff&cs=srgb")',
          }}
        >
  ...
  <Link to="/menu" className="hover:text-[#e2b857] transition-colors relative group">
    Menu
  </Link>
  <Link to="/contact" classNa="hover:text-[#e2b857] transition-colors relative group">
    Contact
  </Link>
</div>
            <motion.button 
      ...
      <section className="px-6 lg:px-20 py-16 bg-[#0c120c]">
        ...
        >
          ...
          <p className="mt-4 max-w-xl text-[16px] text-gray-400 leading-relaxed font-light">
            We bring a trustworthy and professional touch to every event, ensuring your special day is perfect with <span className="text-[#e2b857] font-medium">gold-standard</span> service.
          </p>
        </motion.div>
      ...
      <section className="py-20 text-center bg-[#0f150f] relative overflow-hidden">
        ...
        >
          ...
          <div className="mt-12 flex flex-col items-center">
            ...
            >
              <img src="https://partyslate.imgix.net/photos/1734724/photo-a9678605-fbaf-49bd-8ba0-ea710f19c02f.jpg?ixlib=js-3.8.0&auto=compress%2Cformat&bg=fff&cs=srgb" alt="User" className="w-full h-full object-cover" />
            </motion.div>
            <p className="text-[#e2b857] font-bold text-lg tracking-wide font-serif">David & Sarah</p>
            <p className="text-[12px] text-gray-400 mt-1 uppercase tracking-wider font-medium">Wedding Reception, 2023</p>
          </div>
        </motion.div>
      </section>
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="mt-6 max-w-xl mx-auto text-[16px] text-gray-400 leading-relaxed font-light">
              Contact us today to create a memorable culinary experience for your guests. We handle the details so you can enjoy the party.
            </p>
...
function Stat({ value, label }) {
  return (
    <div className="text-center py-6 md:py-8 hover:bg-white/5 transition-colors duration-300 group cursor-default">
      ...
      <p className="mt-2 text-[10px] md:text-[11px] font-bold uppercase tracking-[0.2em] text-gray-400 group-hover:text-white transition-colors">{label}</p>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="w-12 h-12 rounded-full bg-[#151d29] flex items-center justify-center mb-6 group-hover:scale-110 group-hover:bg-[#e2b857] group-hover:text-black transition-all duration-300">
        {React.cloneElement(icon, {className: "group-hover:text-black transition-colors"
})}
      </div>
      <h3 className="mb-3 font-bold text-[18px] text-white font-serif">{title}</h3>
      <p className="text-[14px] leading-relaxed text-gray-400 font-light">{desc}</p>
    </motion.div>
  );
}