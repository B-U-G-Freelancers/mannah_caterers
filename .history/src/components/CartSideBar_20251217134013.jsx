      ...
      <div className="sticky top-[140px] flex flex-col gap-6">
        ...
        <div className="bg-[#022c1e] rounded-3xl border border-[#24473b] p-6 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex items-center gap-3 mb-8 pt-2">
                <span className="material-symbols-outlined text-[#d4af37] filled">shopping_bag</span>
                <h3 className="text-[#d4af37] font-bold text-xl tracking-wide">Your Menu</h3>
            </div>

            {/* Selected Items List */}
            <div className="flex flex-col gap-1 mb-8">
                {selectedItems.length === 0 ? (
                    <div className="text-[#64748b] text-sm italic py-4">No items selected</div>
                ) : (
                    <div className="max-h-[240px] overflow-y-auto custom-scrollbar -mr-2 pr-2 flex flex-col gap-3">
                        {selectedItems.map((item) => (
                            <div key={item.id} className="flex justify-between items-start group">
                                <span className="text-white font-medium text-sm leading-tight pt-1">{item.name}</span>
                                <button
            ...
            <div className="flex justify-between items-center mb-6">
                 <span className="text-[#cbd5e1] text-sm font-medium">Items Selected:</span>
                 <span className="text-white font-bold text-xl">{selectedItems.length}</span>
            </div>
        ...
        <div className="rounded-2xl border border-dashed border-[#24473b] p-6 text-center bg-[#022c1e]/50">
             <p className="text-[#94a3b8] text-xs leading-relaxed">
                Need help with portions? <button className="text-[#d4af37] font-bold hover:underline">Chat with a chef</button>
             </p>
        </div>
